...
def main(
        ...
        extract_number: int = None, num_samples_used=None,
):
    """
    ...
    save_dir = get_save_dir_name(
        cwd=cwd, mode=mode, num_samples=num_samples if num_samples_used is None else num_samples_used, nn_architecture=nn_architecture,
    ...
    if num_samples_used is not None:
        strain_paths_vector = strain_paths_vector[:num_samples_used]
...
def plot_prediction(y, y_pre, numg_plot=0, s=None, mode=None, nn_architecture=None, extra_description=None):
    ...
    if num_features == 4:
        ...
        plt.scatter(
            ...
            marker='+', color='b', label=r'$\| \epsilon^p \|$ %s' % mode, s=60)
        plt.plot(range(num_step), y_pre[:, 3], label=r'$\| \epsilon^p \|$ pre', color='b', linestyle='--')
        ylabel = r'$\| \epsilon^p \|$'
...
if __name__ == '__main__':
    ...
    """
    J2 2 GRU classical training  rnn_j2_numSamples1000_NNclassic_deps_sig_lc1.0_len200  (show prediction)
                                 rnn_j2_numSamples20_NNclassic_deps_sig_lc1.0_len200  
                                 rnn_j2_numSamples200_NNclassic_deps_sig_lc1.0_len200  
                                 
    J2 1 GRU simple training     rnn_j2_numSamples999_NNsig_deps_sig_simple_lc0.8_len20
    J2 physics-extension         rnn_j2_numSamples201_NNsig_deps_sig_lc1.0_len20
    ...
    # _deps_sig_simple   2 layer GRU unit, no physical expansion introduced
    # _deps_sig  with physical expansion introduced
    # _deps_sig_muLtiGRU
    # _deps_datadriven_sig_epspvec
    # _cal  Calculation of stresses based on J2 model parameters and optimisation of parameters

    extra_description_list = ['_cal']

    # ============================
    num_sample_sensitivity_flag = False
    num_samples_used_list = [20, 200, 1000]
    ...
    # lc = 0.8
    # l_epsp_c = 1.5
    len_sequence = 200 if ('fc' not in extra_description) and ('_mc' not in extra_description) else 2 # 50
    train_flag = True  #
    # train_flag = False
    if train_flag:
        for i in extra_description_list:
            for j in mode_list:
                for num_deep_layer in num_deep_layer_list:
                    for num_samples_used in num_samples_used_list if num_sample_sensitivity_flag else [None]:
                        main(num_samples=999, nn_architecture=nn_architecture, extra_description=i,
                             ...
                             extract_number=extract_number, num_samples_used=num_samples_used)
    else:
        call_and_prediction(
            num_samples=201, nn_architecture=nn_architecture,
            ...
            hidden_size=hidden_size,
            num_samples_used=1000 if num_sample_sensitivity_flag else None)
