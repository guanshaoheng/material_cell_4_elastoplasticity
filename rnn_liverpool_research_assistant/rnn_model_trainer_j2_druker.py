...
if __name__ == '__main__':
    ...
    Druker physics-extension without p  rnn_drucker_numSamples999_NNsig_deps_sig_lc1.0_len10     
    Druker physics-extension with    p  rnn_drucker_numSamples999_NNsig_deps_sig_p_lc1.0_len10
    ...
    mode = 'drucker'  # 'j2' 'drucker'  j2_harden
    mode_list = ['j2']
    ...
    extra_description = '_deps_sig'  # _deps_sig 是带有物理扩展的
    ...
    extra_description_list = ['_deps_sig']

    # ============================
    # 采用以下不同训练样本数目测试网络的泛化能力
    # 得出结论，多少样本数目才足够
    ...
    # lc = 0.8
    # l_epsp_c = 1.5
    len_sequence = 10 if ('fc' not in extra_description) and ('_mc' not in extra_description) else 2 # 50
    train_flag = False  #
    # train_flag = False
    if train_flag:
        for i in extra_description_list:
            for j in mode_list:
                for num_deep_layer in num_deep_layer_list:
                    for num_samples_used in num_samples_used_list:
